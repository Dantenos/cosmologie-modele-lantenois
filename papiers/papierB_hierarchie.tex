...
\section{Heredity instead of teleology: $\beta = 2/(n_{\rm eff}+3)$}\label{sec:heredity}
...
$\beta_1 \simeq -0.42$ (peak, $K=2$) and $-0.56$ (rms, $K=4$), the values the frozen integrator returns. \textbf{Corrected in revision:} an earlier version quoted $-0.9$ to $-1.6$ (rms) against $-0.23$ to $-0.40$ (peak); Paper~A withdraws exactly those figures, and the script that produces them prints the withdrawal in its own output. Refitting Paper~A's
likelihood with $\beta_1$ free gives $\beta_1 = +0.06 \pm 0.31$: the peak prediction costs
$\Delta\chi^2 = 1.5$--$2.5$, the rms prediction $\Delta\chi^2 = 4.0$ (i.e.\ $2.0\sigma$, matching Paper~A). \textbf{Corrected in revision:} an earlier version wrote $\geq 7.5$, which back-solves to $\beta_1 \simeq -0.79$ --- inside the very range this same sentence retracts. \textbf{We therefore replace $\beta = 4/(n_{\rm eff}+3)$ by $\beta = 2/(n_{\rm eff}+3)$},
...
recovered from the matching rather than posited. \emph{(c) An internal-consistency check, and only that}: the relation forces $M_{\rm acc}/M_{\rm tot} = \Omega_{\rm de}$; the fitted values give $2.12/3.09 = 0.686$ against $\Omega_{\rm de} = 0.689$. \textbf{An earlier version presented this as a consistency check ``that was not designed as one'', obtained independently a week apart. That framing is withdrawn}: given $\rho_{de} = M_{\rm acc}/V$ and $\rho_{\rm tot} = M_{\rm tot}/V$, the ratio \emph{is} $\Omega_{\rm de}$ identically. The $0.4\%$ measures our rounding, not the physics, and we should not have counted it as evidence. What remains genuinely open: the matching \emph{assumes} an exactly FRW
...
$\Phi_{\rm int} \propto \epsilon\,(r/R)^\ell/(2\ell+1)$: at $\ell = 2$ and $r/R \simeq 0.98$ this transmits with factor $0.98^2/5 = 0.192$. \textbf{Corrected in revision:} an earlier version quoted $0.58$, a factor three larger, which does not follow from the formula as written; the resulting bound below loosens accordingly, from $\epsilon \lesssim 0.35$ to $\epsilon \lesssim 1.0$. We also note that the same formula returns $1$, not $0$, at $\ell = 0$, so ``the monopole is screened entirely'' does not follow from it either --- the shell theorem screens the monopole, but this expression does not encode that, and the derivation as stated is incomplete. Requiring the induced quadrupole not to exceed the observed large-angle
anomalies ($\lesssim 20\%$) bounds the parent's accretion asymmetry at $\epsilon \lesssim 1.0$ ($0.20/0.192$; the $0.35$ an earlier version printed here is $0.20/0.58$, i.e.\ the retracted transmission factor).
...
(iii) \emph{The generational closure}: the heredity relation $\beta = 2/(n_{\rm eff}+3)$ maps a
...
mass --- it is defined by $n_{\rm eff}(M_{\rm cut}) = -2.540$ under the adopted relation ($-2.08$ was its image under the superseded one) --- so it moves with the parameters,
...
Developed with AI assistance. All numerical claims are reproducible with the Paper-A pipeline.
...
\bibitem{Diemand2004} J.~Diemand et al. \emph{[no title, year or identifier --- flagged by the manuscript check; this entry must be completed or the citation dropped]}; see also A.~Del Popolo, on the radial shape of peaks being steeper than the correlation function.
