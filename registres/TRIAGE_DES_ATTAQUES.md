...
| 80 | mon test du référent, v3 (#200) | **ARRÊTÉ SUR SA VALIDATION A**, et la cause est moi : j'ai gelé « reproduire exactement les valeurs de grille » AVEC un estimateur qui abandonne la grille. **Mode 6 de ma propre taxonomie, écrit après l'avoir catalogué.** L'exécution établit pourtant que l'estimateur parabolique marche, qu'il n'a aucun plancher (σ ×2,00 puis ×4,00 exactement) et qu'il condamne indépendamment la même configuration que le contrôle de forme du #190 (Δχ²(−σ) NÉGATIF). |
| 81 | la série elle-même : trois conceptions, trois blocages | **DÉCISION D'ARRÊT (#200)** : une v4 tolérant un pas de grille passerait, et ce serait du p-hacking au niveau du protocole. Le gel protège chaque étude, PAS la série. 11e mode de la taxonomie ; règle d'arrêt adoptée : **au troisième blocage sur une même question, on cesse et on consigne le statut non établi.** La lecture « référent » du #193 reste NON ÉTABLIE. |

**Bilan : 10 attaques justes, 72 affirmations réfutées, rétrogradées ou corrigées — dont, pour la première fois, une décision d'arrêt contre soi-même.**

| 82 | mon papier F : « neuf façons NOUVELLES dont un critère pré-enregistré échoue » | **AUCUNE N'EST NOUVELLE (#201)** : mon mode 8 est la **vacuité**, nommée en 1997 (Beer et al., CAV), dont l'exemple canonique est littéralement le mien ; mode 7 = incohérence code/commentaire ; mode 5 = Feldman & Cousins 1998 ; mode 6 = spécification insatisfiable ; mode 9 = test instable ; modes 3 et 4 n'en font qu'un. Papier réécrit comme **cartographie** : onze instances observées sur sept classes nommées. |
| 83 | mon protocole de gel par empreinte, présenté comme un protocole | **RÉ-EMBALLAGE (#201)** : Design by Contract 1986, OSF Registrations, OriginStamp, Beaulieu-Jones & Greene 2017 pour la CI, et **une centaine de dépôts cosmologie déjà SHA-256 + OpenTimestamps**. Rétrogradé en note de conception ; ne survit qu'un invariant : **geler l'oracle, pas l'implémentation**. |
