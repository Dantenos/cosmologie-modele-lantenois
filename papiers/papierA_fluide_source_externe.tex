...
from each other: they differ by only $0.14$ in $\beta_1$, so telling them apart at $3\sigma$
requires $\sigma(\beta_1) \simeq 0.047$ --- recomputed from the corrected pair; the $0.20$ and $0.07$ an earlier version gave were the withdrawn pair's arithmetic, and the correction makes this test \emph{harder}, not easier. The running tests whether $\beta$ runs at all, not which
...
$\kappa = +0.200 \pm 0.128$ and the rms reading $\kappa = +0.258 \pm 0.128$, i.e.\ $1.5\sigma$ and
...
clock at the $1.5$--$2.0\sigma$ level, or else leaves heredity in tension with the data at the
...
$\beta_1 \simeq -0.42$ at the seed scale, a curvature $\beta_2 \simeq +0.12$ --- a factor $2.6$
...
inherited $n_{\rm eff} = 2/\beta - 3$ & $-2.174$ & $-2.219$ & shifts seed scale\\
$F_{\rm AP}(2.33)$ predicted & $4.492$ ($-1.7\sigma$) & $4.515$ ($-1.2\sigma$) & \emph{defeat eases}\\
$k_{\rm eff}(z=0) = -3w_0$ & $2.56$ & $2.69$ & vs.\ Farrah $3.09 \pm 0.76$: $-0.7\sigma$ / $-0.5\sigma$\\
...
$-1\%$ to $+15\%$: \textbf{this model neither alleviates nor aggravates the ``why now'' coincidence}.
...
\paragraph{P3, sharpened.} Since the component has no internal degrees of freedom, it carries no entropy perturbations: its rest-frame perturbations must follow the adiabatic mode, $c_a^2 = w - \dot w/[3H(1+w)]$, which \emph{diverges at the phantom crossing} $z_\times \simeq 0.22$--$0.44$ (depending on $\beta$ and $\Omega_m$; the single value $0.36$ quoted in an earlier draft is withdrawn, see the verification pass). The fluid description of perturbations is therefore necessarily incomplete at the crossing, and the microphysics (torsion production or interior coupling) must specify the crossing behaviour --- precisely what the PPF prescription papers over phenomenologically. This is now a sharp, well-posed theoretical requirement rather than a vague debt.
...
figure is wrong even at its own exponent --- the frozen computation gives $0.4436$ at $\beta = 2.42$, the SN+BAO value --- and the crossing is in any case not stable; it is not stable across the measured range:
\begin{center}
\begin{tabular}{lccc}
\hline
...
$\Omega_b h_{70} = 0.0357$: a $4.4\sigma$ deficit (corrected in revision from $0.0343$ and $4.9\sigma$, which
require a suppression factor $0.700$ rather than the $0.729$ stated in the same sentence). Our own model gives $0.0470$, i.e.\ $0.7\sigma$,
...
\end{tabular}
\end{center}
\emph{The test is therefore worth about $2.1\sigma$, not the $4.4\sigma$ a fixed-nuisance reading gives}: the nuisances absorb most of
...
mapping forces $w$ constant at $-0.853$ out to $z=3$, which is not our model and is excluded for
...
\emph{Numbering note.} The predictions below are labelled P1--P8 and P10: there is deliberately no P9. The label P10 is retained rather than renumbered because it is referenced by number in the companion papers and in the repository's audit trail (\#145); the gap is recorded here so it is not read as a typographical omission.
\begin{itemize}
\item[P1.] \emph{Rigid shape.} $w(z) = -(\beta/3)/(Ht)$ is a specific one-parameter curve, nonlinear in $(1-a)$. Nonparametric reconstructions of $w(z)$ (DESI~DR3, Euclid, LSST) that depart from this family falsify the power-law variant. A public fitting pipeline accompanies this note, and the DR3 analysis protocol --- data format, thresholds, and the three exhaustive verdicts --- is sealed \emph{in advance} of the data: the arbiter file is pinned by its hash, $\mathrm{sha256}(\texttt{scelle.py}) = \texttt{68d06bcc}\ldots$, published in the repository, archived under Zenodo \textsc{doi}~\texttt{10.5281/zenodo.22095425} (an independent timestamp obtained before DR3), and enforced by its continuous integration, so that anyone can verify in 2027 that the analysis did not move after seeing the data.
