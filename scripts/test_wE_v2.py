...
ZSTAR = 1089.91
# CMB comprime
OB_C, OB_S = 0.02236, 0.00015
OM_C, OM_S = 0.1430,  0.0011
DMS_C, DMS_S = 94.31, 0.30
    ...
    for i, ob in enumerate(obs):
        for j, om in enumerate(oms):
            ...
            p = camb.set_params(H0=67.5, ombh2=ob, omch2=och2, mnu=0.06, tau=0.054,
                                As=2.1e-9, ns=0.965)
            R[i, j] = camb.get_background(p).get_derived_params()['rdrag']
    return RectBivariateSpline(obs, oms, R, kx=3, ky=3)
RD = build_rd()
def r_drag(ob, om): return float(RD(ob, om)[0, 0])
...
def chi2(h, ob, Om, wE, par, famille):
    ...
    # CMB comprime + SH0ES
    dms = np.interp(ZSTAR, zg, Dc)*DH0/rd
    c_cmb = ((ob-OB_C)/OB_S)**2 + ((om-OM_C)/OM_S)**2 + ((dms-DMS_C)/DMS_S)**2
