...
three quantities ($\Xi = 1.382$ against $1.403$; survival fraction $0.702$ against $0.70$; a \emph{derived} $H_0 = 69.61$ against $69.94$) --- \textbf{of which only the first is independent}: the rate normalisation is solved so that the last two are returned (\S3), and calling all three independent, as an earlier version of this abstract did, overstated the validation. We add that the frozen criterion on the \emph{derived} $\Xi$ currently fails at $53\%$ ($\Xi = 2.149$), which \S4 reports and this abstract previously did not. On Pantheon+, DESI~DR2 and the Planck distance priors the two models are statistically
