...
moving between $67.97$ and $70.91$ as $z_i$ runs over $[10,30]$ \emph{at fixed} $(\omega_c, \Xi)$. Once $\Xi$ is
re-fitted at each $z_i$ --- which the coupled model is entitled to --- the derived $H_0$ moves only between
$70.22$ and $70.43$ ($\Xi = 3.97$--$4.53$ absorbs the onset), and $\Delta\mathrm{AIC}$ against the accretion
model ranges over $+3.5$ to $+6.1$: the onset of first light is a hidden parameter of the coupled model, but
one that $\Xi$ absorbs, not a free lever on $H_0$. (The sensitivity test in an earlier version of the script
was inoperative --- $z_i$ frozen at function definition --- and the fixed-parameter figures were the only ones
reported; corrected in the audit trail, entry \#143.)
