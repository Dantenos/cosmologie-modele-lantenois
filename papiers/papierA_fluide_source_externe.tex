...
matched today. Integrating it gives $\beta_1 = -0.45 \pm 0.03$ for the peak reading ($K=2$) and $-0.68$ for the
rms reading ($K=4$) --- point values, not ranges, the quoted spread coming from the range of
...
routes span $-0.45$, $-0.68$, $-1.95$; the measurement is $+0.06 \pm 0.31$. \emph{All three
predicted runnings are negative and the data prefer zero}, at $1.6\sigma$, $2.4\sigma$ and
...
$\kappa = +0.21 \pm 0.12$ and the rms reading $\kappa = +0.28 \pm 0.12$, i.e.\ $1.6\sigma$ and
$2.3\sigma$ from zero respectively. \emph{Taking heredity seriously therefore requires a curved
clock at the $1.6$--$2.3\sigma$ level, or else leaves heredity in tension with the data at the
...
\paragraph{Coincidence window: the model is marginally \emph{worse} than $\Lambda$CDM (added in revision).}
...
= 6.4$--$7.0$ here across the allowed $\beta$ range, against $6.0$ for $\Lambda$CDM computed
...
in its recalibrated \emph{Dovekie} release (1820 SNe, full stat+sys) gives: DES alone
...
\emph{(vi) The LRG double-removal test, and how our compilation ordering compares with the
...
the combination analysis (arXiv:2504.15336), which argues one may neither select nor average these
...
\emph{(iv) Tension null model, with the double-counting removed.} Letting the low-$z$ offset float
...
significance: a $2.4$-unit gain for one parameter is a $\sim 1.2\sigma$ effect ($p \simeq 0.12$),
...
\emph{(iv) BAO jackknife, low-$z$ and full-Planck.} Removing each of the 13 BAO measurements in
...
\emph{(v) Earlier BAO jackknife note.} Removing
each of the 13 BAO measurements in turn leaves $\Delta\chi^2 \in [-5.67, -3.50]$ and
$\beta \in [2.42, 2.49]$: no single BAO point carries the low-$z$ result, and the two LRG $D_M$
points most often suspected of driving the DESI signal in fact \emph{resist} this model (removing
them improves the fit). Conclusion: the shape and its exponent are robust; the low-$z$ evidence is
conditional on supernova calibration and should be read as such. The full-Planck result quoted above
rests on a different data combination and requires its own jackknife, which we flag as outstanding.
...
The observable prediction: $f\sigma_8$ is suppressed relative to $\Lambda$CDM by $1.3$--$1.9\%$ over
$z = 0.15$--$0.5$, crossing to a $\sim 1\%$ enhancement by $z = 1.5$ --- the characteristic signature of
...
(added in revision). Our $S_8 = 0.826$, against $\Lambda$CDM's $0.810$, sits at $0.6\sigma$ from
KiDS-Legacy ($0.815^{+0.016}_{-0.021}$), $1.4\sigma$ from ACT~DR6 ($0.790^{+0.024}_{-0.027}$) and
...
$2.49$ & post-verification & $-0.874$ & $0.345$ \\
$2.56$ & full-Planck profile & $-0.898$ & $0.253$ \\
$2.60$ & distance priors & $-0.910$ & $0.214$ \\
\hline
\end{tabular}
\end{center}
A $7\%$ change in $\beta$ moves the crossing by a factor of two. \emph{We therefore withdraw
...
degenerate with it), $0.20$ (Anton--Schmidt), $0.31$ (particle creation, PC1) --- all within
$0.1$ of each other, with present-day equations of state between $-0.91$ and $-0.95$. The
...
The onset of first light, which we fix at $z_i = 20$, moves the derived $H_0$ between $67.97$ and
$70.91$ for $z_i \in [10,30]$: it is a hidden parameter of the rival, not a fitted one. And the
...
$69.94$, obtained from entirely different data. The implementation is validated.
...
\bibitem{AntonSchmidt2026} \emph{Testing the Anton--Schmidt dark energy model with the DESI DR2 measurements}, Phys. Rev. D (2026).
\bibitem{IDSFT2026} \emph{Interacting dark sector field theory with phantom crossing}, arXiv:2605.20060.
\bibitem{NoGo2026} \emph{Evidence for evolving dark energy from DESI DR2 BAO and Pantheon+, DES-Dovekie, and Union3}, Eur. Phys. J. C \textbf{86} (2026), doi:10.1140/epjc/s10052-026-15806-w.
...
\bibitem{Croker2024DESI} K. S. Croker et al., \emph{DESI Dark Energy Time Evolution is Recovered by Cosmologically Coupled Black Holes}, JCAP \textbf{10} (2024) 094 [arXiv:2405.12282].
\end{thebibliography}
